...
> 一、sort、arraycopy，ok
>
> 二、借助 PriorityQueue 的大顶堆求 topK，perfect
>
> 三、使用快排算法的变形求 topk，perfect

```java
class Solution {
    public int[] getLeastNumbers(int[] arr, int k) {
        int len = arr.length;
        if (k == 0 || len < k) return new int[0];

        // 使用大顶堆, 小于堆顶的数才能入堆, 同时更新堆顶
...
> 一、动态规划，舍弃对 dp[i] 为负贡献的 dp[i-1]，使用 max 变量保存连续子数组的最大和，ok
>
> 二、贪心，当叠加的和小于等于0时，就从下一个数重新开始重置叠加和、当叠加和大于0时，将下一个数值加入叠加和，使用 max 变量保存连续子数组的最大和，perfect
